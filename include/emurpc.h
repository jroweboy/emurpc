...
struct emurpc_params_create_overlay {
    ...
    uint16_t x; /// X Position of the overlay starting from the top left of the
                /// render window
    uint16_t y; /// Y Position of the overlay starting from the top left of the
                /// render window
    uint32_t width;
    uint32_t height;
};
typedef bool (*emurpc_callback_create_overlay)(
    void* user_data, struct emurpc_params_create_overlay);
...
typedef bool (*emurpc_callback_draw_overlay_text)(
    void* user_data, struct emurpc_params_draw_overlay_text);
...
bool emurpc_check_addr_range(struct emurpc_state*, uint64_t addr,
                             uint64_t size);
